...
def getCharmanderLink(soup):
    ...
    info_tag = tag.find_all('div', classe_='infocard-lg-img')[3]
    results = info_tag.find('a')['href']


    return 'https://pokemondb.net' + results
...
def getEggMoves(pokemon):
    ...
    moveslist = []
    a = soup.find_all('table', classe_ = 'data-table')[2]
    a2 = a.find_all('a', class_ = 'ent-name')
    for move in a2:
        moveslist.append(move.text)
    return movestlist
